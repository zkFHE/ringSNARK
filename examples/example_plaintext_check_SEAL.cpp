...
int main() {
    ...
    // Set public values
    ringsnark::pb_variable_array<R> vars(num_variables, ringsnark::pb_variable<R>());
    vars.allocate(pb, num_variables, "x");
    pb.set_input_sizes(num_variables - 1); // plaintext is public, bit-decomposition is private

    // Set constraints
    ringsnark::linear_combination<R> sum = 0;
    for (size_t i = 0; i < logT; i++) {
      pb.add_r1cs_constraint(ringsnark::r1cs_constraint<R>(vars[i], 1-vars[i], 0));
      sum = sum + (1<<i) * vars[i];
    }
    pb.add_r1cs_constraint(ringsnark::r1cs_constraint<R>(vars[num_variables-1], 1, sum));
    ...
    // Set values
    {
        ...
        // This convoluted way of initializing x is needed to trick polytools into thinking that
        // the polynomial is in NTT form (it's not).
        ...
        for (size_t i = 0; i < logT; i++) {
          ...
          const polytools::SealPoly bits_poly(context, bits, &(context.first_parms_id()));
