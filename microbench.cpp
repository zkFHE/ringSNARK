...
int main() {
    ...
    cout << "[TIME][CLIENT] One-time setup\t" << chrono::duration_cast<chrono::microseconds>(end - start).count()
         << " us" << endl;
    ...
    unsigned long long size = 9 * x_encrypted.size() * parms.coeff_modulus().size() * parms.poly_modulus_degree() * 8;
