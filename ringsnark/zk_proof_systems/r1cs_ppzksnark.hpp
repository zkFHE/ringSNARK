 ...
 *****************************************************************************
 ...
 Declaration of interfaces for a pre-processing zero-knowledge SNARK (ppzkSNARK)
 for R1CS.
...
template <typename RingT, typename EncT>
bool operator==(const proving_key<RingT, EncT> &lhs,
                const proving_key<RingT, EncT> &rhs);
...
template <typename RingT, typename EncT>
std::ostream &operator<<(std::ostream &out,
                         const processed_verification_key<RingT, EncT> &pvk);

template <typename RingT, typename EncT>
std::istream &operator>>(std::istream &in,
                         processed_verification_key<RingT, EncT> &pvk);
...
/**
 * A key pair for the R1CS ppzkSNARK, which consists of a proving key and a
 * verification key.
 */
//    template<typename RingT, typename EncT>
//    using keypair = std::tuple<proving_key<RingT, EncT>,
//    verification_key<RingT, EncT>>;
 ...
 *
 * Given a R1CS constraint system CS, this algorithm produces proving and
 * verification keys for CS.
 */
template <typename RingT, typename EncT>
 ...
 */
template <typename RingT, typename EncT>
processed_verification_key<RingT, EncT> verifier_process_vk(
    const verification_key<RingT, EncT> &vk);
 ...
 */
template <typename RingT, typename EncT>
bool online_verifier_strong_IC(
    const processed_verification_key<RingT, EncT> &pvk,
    const r1cs_primary_input<RingT> &primary_input,
    const proof<RingT, EncT> &proof);
