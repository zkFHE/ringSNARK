...
keypair<RingT, EncT> generator(const r1cs_constraint_system<RingT> &cs) {
  ...
  // TODO: get inverse directly from generation, since it has to be computed
  // anyway?
  ...
  auto pk = new proving_key<RingT, EncT>(
      cs, EncT::encode(sk_enc, {alpha})[0], EncT::encode(sk_enc, {beta})[0],
      EncT::encode(sk_enc, s_pows_ring), EncT::encode(sk_enc, gamma_io),
      EncT::encode(sk_enc, delta_mid), EncT::encode(sk_enc, delta_ts_ring),
      pk_enc);

  auto vk = new verification_key<RingT, EncT>(*pk, s, alpha, beta, gamma, delta,
                                              sk_enc);
...
proof<RingT, EncT> prover(const proving_key<RingT, EncT> &pk,
  ...
  const qrp_witness<RingT> qrp_wit = r1cs_to_qrp_witness_map(
      pk.constraint_system, primary_input, auxiliary_input, RingT::zero(),
      RingT::zero(), RingT::zero());

  // TODO: this is highly non-optimized, skip all the zero-multiplication
  // s_pows have length d+1, where d = cs.num_constraints() is the size of the
  // QRP
  ...
  if (!auxiliary_input.empty()) {
    c_enc +=
        EncT::inner_product(pk.delta_mid.begin(), pk.delta_mid.end(),
...
bool verifier(const verification_key<RingT, EncT> &vk,
  ...
  // TODO: do we need to chek on encodings, or can we only do checks on
  // plaintexts/ring elements? If the latter, can  we simplify the gamma * (...)
  // / gamma term in rhs?
