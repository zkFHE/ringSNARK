...
class proving_key : public ringsnark::proving_key<RingT, EncT> {
 ...
 public:
  ...
  proving_key(const ringsnark::r1cs_constraint_system<RingT> &constraint_system,
              const EncT alpha, const EncT beta, const vector<EncT> &s_pows,
              const vector<EncT> &gamma_io, const vector<EncT> &delta_mid,
              const vector<EncT> &delta_ts, const PublicKey &pk_enc)
...
class verification_key : public ringsnark::verification_key<RingT, EncT> {
 ...
 public:
  ...
  verification_key(const proving_key<RingT, EncT> &pk, RingT s, RingT alpha,
                   RingT beta, RingT gamma, RingT delta, SecretKey sk_enc)
      : pk(pk),
        s(s),
        alpha(alpha),
        beta(beta),
  ...
  [[nodiscard]] size_t size_in_bits() const override {
    return s.size_in_bits() + alpha.size_in_bits() + beta.size_in_bits() +
           gamma.size_in_bits() + delta.size_in_bits() +
...
class keypair {
 public:
  ...
  keypair(const proving_key<RingT, EncT> &pk,
          const verification_key<RingT, EncT> &vk)
      : pk(pk), vk(vk) {}
