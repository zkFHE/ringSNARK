...
class proving_key : public ringsnark::proving_key<RingT, EncT> {
 ...
 public:
  ...
  proving_key(const ringsnark::r1cs_constraint_system<RingT> &constraint_system,
              const vector<EncT> &s_pows, const vector<EncT> &alpha_s_pows,
              const vector<EncT> &beta_prods, const EncT beta_rv_ts,
              const EncT beta_rw_ts, const EncT beta_ry_ts,
              const EncT alpha_rv_ts, const EncT alpha_rw_ts,
              const EncT alpha_ry_ts, const vector<EncT> rv_vs,
              const vector<EncT> rw_ws, const vector<EncT> ry_ys,
              const PublicKey &pk_enc)
      : constraint_system(constraint_system),
        s_pows(s_pows),
        alpha_s_pows(alpha_s_pows),
        beta_prods(beta_prods),
        beta_rv_ts(beta_rv_ts),
        beta_rw_ts(beta_rw_ts),
        beta_ry_ts(beta_ry_ts),
        alpha_rv_ts(alpha_rv_ts),
        alpha_rw_ts(alpha_rw_ts),
        alpha_ry_ts(alpha_ry_ts),
        rv_vs(rv_vs),
        rw_ws(rw_ws),
        ry_ys(ry_ys),
        pk_enc(pk_enc) {
  ...
  [[nodiscard]] size_t size_in_bits() const override {
    return s_pows.size() * s_pows[0].size_in_bits() +
           ...
           beta_rv_ts.size_in_bits() + beta_rw_ts.size_in_bits() +
           beta_ry_ts.size_in_bits() + EncT::size_in_bits_pk(pk_enc);
  }
};
...
class verification_key : public ringsnark::verification_key<RingT, EncT> {
 ...
 public:
  ...
  verification_key(const proving_key<RingT, EncT> &pk, RingT s, RingT alpha,
                   RingT beta, RingT r_v, RingT r_w, RingT r_y,
                   SecretKey sk_enc)
      : pk(pk),
        s(s),
        alpha(alpha),
...
class keypair {
 public:
  ...
  keypair(proving_key<RingT, EncT> &pk, const verification_key<RingT, EncT> &vk)
      : pk(pk), vk(vk) {}

  keypair(proving_key<RingT, EncT> &&pk, verification_key<RingT, EncT> &&vk)
...
class proof : public ringsnark::proof<RingT, EncT> {
 public:
  ...
  proof(EncT a, EncT aPrime, EncT b, EncT bPrime, EncT c, EncT cPrime, EncT d,
        EncT dPrime, EncT f)
      : A(a),
        A_prime(aPrime),
        B(b),
  ...
  [[nodiscard]] size_t size_in_bits() const override {
    return A.size_in_bits() + A_prime.size_in_bits() + B.size_in_bits() +
           B_prime.size_in_bits() + C.size_in_bits() + C_prime.size_in_bits() +
           D.size_in_bits() + D_prime.size_in_bits() + F.size_in_bits();
  }
};
}  // namespace ringsnark::rinocchio
