 ...
 *
 * Namely, given a R1CS constraint system cs, construct a QRP instance for
 * which: A := (A_0(z),A_1(z),...,A_m(z)) B := (B_0(z),B_1(z),...,B_m(z)) C :=
 * (C_0(z),C_1(z),...,C_m(z)) where m = number of variables of the QRP and each
 * A_i,B_i,C_i is expressed in the Lagrange basis.
 */
...
qrp_instance<RingT> r1cs_to_qrp_instance_map(
  ...
  std::vector<std::map<size_t, RingT>> A_in_Lagrange_basis(cs.num_variables() +
                                                           1);
  std::vector<std::map<size_t, RingT>> B_in_Lagrange_basis(cs.num_variables() +
                                                           1);
  std::vector<std::map<size_t, RingT>> C_in_Lagrange_basis(cs.num_variables() +
                                                           1);
  ...
  return qrp_instance<RingT>(domain, cs.num_variables(), domain->m,
                             cs.num_inputs(), std::move(A_in_Lagrange_basis),
...
qrp_instance_evaluation<RingT> r1cs_to_qrp_instance_map_with_evaluation(
  ...
  return qrp_instance_evaluation<RingT>(
      domain, cs.num_variables(), domain->m, cs.num_inputs(), t, std::move(At),
      std::move(Bt), std::move(Ct), std::move(Ht), Zt);
}
 ...
 *  (6) patch H to account for d1,d2,d3 (i.e., add coefficients of the
 * polynomial (A d2 + B d1 - d3) + d1*d2*Z )
 *
...
qrp_witness<RingT> r1cs_to_qrp_witness_map(
  ...
  r1cs_variable_assignment<RingT> auxiliary_assignment(primary_input.size(),
                                                       RingT::zero());
  auxiliary_assignment.insert(auxiliary_assignment.end(),
  ...
  primary_assignment.insert(primary_assignment.end(), zeros.begin(),
                            zeros.end());
  ...
  std::vector<RingT> aA(domain->m, RingT::zero()), aB(domain->m, RingT::zero()),
      aC(domain->m, RingT::zero());
  for (size_t i = 0; i < cs.num_constraints(); ++i) {
...
#pragma omp parallel for
#endif
  for (size_t i = 0; i < std::min(coefficients_for_H.size(), H_tmp.size());
       ++i) {
