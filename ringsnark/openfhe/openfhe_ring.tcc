...
namespace ringsnark::openfhe {
    ...
    RingElem::RingElem(const Poly &poly) : value(Poly(poly)) {}
...
//                }
    ...
    bool operator==(const RingElem &lhs, const RingElem &rhs) {
        if (lhs.is_scalar() && rhs.is_scalar()) {
            ...
            return lhs.to_poly().get_poly() == rhs.get_poly();
        } else if (lhs.is_poly() && rhs.is_scalar()) {             // TODO: cast down to scalar instead?
            return lhs.get_poly() == rhs.to_poly().get_poly();
        } else {
...
//            return out << elem.get_poly().to_json(); // TODO
        } else {
            throw RingElem::invalid_ring_elem_types();
        }
    }
...
//
//        for (const auto &r: rs) {
//            ::Poly poly = (r.is_scalar()) ? r.to_poly().get_poly() : r.get_poly();
//
...
//        return RingElem(Poly(RingElem::get_context(), coeffs, &parms.parms_id()));
    }
