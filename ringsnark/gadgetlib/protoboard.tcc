...
template <typename RingT>
var_index_t protoboard<RingT>::allocate_var_index(
...
template <typename RingT>
void protoboard<RingT>::add_r1cs_constraint(
    const r1cs_constraint<RingT> &constr, const std::string &annotation) {
#ifdef DEBUG
  assert(annotation != "");
  constraint_system
...
template <typename RingT>
void protoboard<RingT>::augment_variable_annotation(
    const pb_variable<RingT> &v, const std::string &postfix) {
#ifdef DEBUG
...
template <typename RingT>
r1cs_variable_assignment<RingT> protoboard<RingT>::full_variable_assignment()
    const {
  return values;
}
...
r1cs_primary_input<RingT> protoboard<RingT>::primary_input() const {
  return r1cs_primary_input<RingT>(values.begin(),
                                   values.begin() + num_inputs());
}

template <typename RingT>
r1cs_auxiliary_input<RingT> protoboard<RingT>::auxiliary_input() const {
  return r1cs_auxiliary_input<RingT>(values.begin() + num_inputs(),
                                     values.end());
}
