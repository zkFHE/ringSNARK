...
class protoboard {
 private:
  RingT constant_term; /* only here, because pb.val() needs to be able to return
                          reference to the constant 1 term */
  r1cs_variable_assignment<RingT>
      values; /* values[0] will hold the value of the first allocated variable
 ...
 public:
  ...
  void add_r1cs_constraint(const r1cs_constraint<RingT> &constr,
                           const std::string &annotation = "");

  void augment_variable_annotation(const pb_variable<RingT> &v,
