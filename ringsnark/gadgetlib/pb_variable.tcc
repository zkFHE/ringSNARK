...
void pb_variable<RingT>::allocate(protoboard<RingT> &pb,
...
template <typename RingT>
void pb_variable_array<RingT>::allocate(protoboard<RingT> &pb, const size_t n,
...
template <typename RingT>
void pb_variable_array<RingT>::fill_with_field_elements(
    protoboard<RingT> &pb, const std::vector<RingT> &vals) const {
...
// template<typename RingT>
// void pb_variable_array<RingT>::fill_with_bits(protoboard<RingT> &pb, const
// libff::bit_vector& bits) const
//{
...
template <typename RingT>
void pb_variable_array<RingT>::fill_with_bits_of_field_element(
    protoboard<RingT> &pb, const RingT &r) const {
  //    const libff::bigint<RingT::num_limbs> rint = r.as_bigint();
  //    for (size_t i = 0; i < this->size(); ++i)
  //    {
  //        pb.val((*this)[i]) = rint.test_bit(i) ? RingT::one() :
  //        RingT::zero();
  //    }
}

template <typename RingT>
void pb_variable_array<RingT>::fill_with_bits_of_ulong(
    protoboard<RingT> &pb, const unsigned long i) const {
  this->fill_with_bits_of_field_element(pb, RingT(i, true));
}

template <typename RingT>
std::vector<RingT> pb_variable_array<RingT>::get_vals(
    const protoboard<RingT> &pb) const {
...
//
// template<typename RingT>
// libff::bit_vector pb_variable_array<RingT>::get_bits(const protoboard<RingT>
// &pb) const
//{
...
template <typename RingT>
RingT pb_variable_array<RingT>::get_field_element_from_bits(
    const protoboard<RingT> &pb) const {
...
template <typename RingT>
void pb_linear_combination<RingT>::assign(protoboard<RingT> &pb,
...
template <typename RingT>
void pb_linear_combination_array<RingT>::fill_with_field_elements(
    protoboard<RingT> &pb, const std::vector<RingT> &vals) const {
  assert(this->size() == vals.size());
  for (size_t i = 0; i < vals.size(); ++i) {
...
// template<typename RingT>
// void pb_linear_combination_array<RingT>::fill_with_bits(protoboard<RingT>
// &pb, const libff::bit_vector& bits) const
//{
//     assert(this->size() == bits.size());
//     for (size_t i = 0; i < bits.size(); ++i)
//     {
...
template <typename RingT>
void pb_linear_combination_array<RingT>::fill_with_bits_of_field_element(
    protoboard<RingT> &pb, const RingT &r) const {
  //    const libff::bigint<RingT::num_limbs> rint = r.as_bigint();
  //    for (size_t i = 0; i < this->size(); ++i)
  //    {
  //        pb.lc_val((*this)[i]) = rint.test_bit(i) ? RingT::one() :
  //        RingT::zero();
  //    }
}

template <typename RingT>
void pb_linear_combination_array<RingT>::fill_with_bits_of_ulong(
    protoboard<RingT> &pb, const unsigned long i) const {
  this->fill_with_bits_of_field_element(pb, RingT(i));
}

template <typename RingT>
std::vector<RingT> pb_linear_combination_array<RingT>::get_vals(
    const protoboard<RingT> &pb) const {
  std::vector<RingT> result(this->size());
  for (size_t i = 0; i < this->size(); ++i) {
...
// template<typename RingT>
// libff::bit_vector pb_linear_combination_array<RingT>::get_bits(const
// protoboard<RingT> &pb) const
//{
//     libff::bit_vector result;
//     for (size_t i = 0; i < this->size(); ++i)
//     {
...
template <typename RingT>
RingT pb_linear_combination_array<RingT>::get_field_element_from_bits(
    const protoboard<RingT> &pb) const {
  RingT result = RingT::zero();
...
template <typename RingT>
linear_combination<RingT> pb_packing_sum(
    const pb_linear_combination_array<RingT> &v) {
...
template <typename RingT>
linear_combination<RingT> pb_coeff_sum(
    const pb_linear_combination_array<RingT> &v,
    const std::vector<RingT> &coeffs) {
