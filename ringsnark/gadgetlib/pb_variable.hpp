...
class pb_variable_array : private std::vector<pb_variable<RingT> > {
 ...
 public:
  ...
  pb_variable_array(size_t count, const pb_variable<RingT> &value)
      : contents(count, value){};
  pb_variable_array(typename contents::const_iterator first,
  ...
  pb_variable_array(typename contents::const_reverse_iterator first,
      ...
      : contents(first, last){};
  void allocate(protoboard<RingT> &pb, const size_t n,
                const std::string &annotation_prefix = "");

  void fill_with_field_elements(protoboard<RingT> &pb,
                                const std::vector<RingT> &vals) const;
  //    void fill_with_bits(protoboard<RingT> &pb, const libff::bit_vector&
  //    bits) const;
  void fill_with_bits_of_ulong(protoboard<RingT> &pb,
                               const unsigned long i) const;
  void fill_with_bits_of_field_element(protoboard<RingT> &pb,
                                       const RingT &r) const;
...
class pb_linear_combination_array
    : private std::vector<pb_linear_combination<RingT> > {
 ...
 public:
  ...
  pb_linear_combination_array(const pb_variable_array<RingT> &arr) {
    for (auto &v : arr) this->emplace_back(pb_linear_combination<RingT>(v));
  };
  pb_linear_combination_array(size_t count) : contents(count){};
  pb_linear_combination_array(size_t count,
                              const pb_linear_combination<RingT> &value)
      : contents(count, value){};
