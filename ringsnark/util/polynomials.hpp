 ...
 * @param x vector of n evaluation points
 * @param y vector of n evaluations y[i] = f(x[i]), for a degree-(n-1)
 * polynomial f
...
/**
 * Return quotient of polynomial division of `numerator' by `denominator' over
 * the ring RingT. The result is a vector of coefficients of size
 * numerator.size() - denominator.size() (padded with zeros if the quotient
 * polynomial has a degree less than this size).
 * @tparam RingT
 * @param numerator
 * @param denominator
 * @return
