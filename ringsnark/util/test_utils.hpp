...
class PrimitiveWrapper {
 public:
  ...
  bool operator==(const PrimitiveWrapper<T> &rhs) const {
    return abs(val - rhs.val) <=
           1e-3 * abs(val);  // Approximate equality for double
  }
...
template <typename T>
inline PrimitiveWrapper<T> operator+(PrimitiveWrapper<T> lhs,
                                     const PrimitiveWrapper<T> &rhs) {
  lhs += rhs;
  return lhs;
}
