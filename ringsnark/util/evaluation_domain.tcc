...
evaluation_domain<RingT>::evaluation_domain(const size_t m)
    : m(m), values(vector<RingT>(m)) {
...
template <typename RingT>
vector<RingT> evaluation_domain<RingT>::evaluate_all_lagrange_polynomials(
    const RingT &t) const {
...
template <typename RingT>
RingT evaluation_domain<RingT>::compute_vanishing_polynomial(
    const RingT &t) const {
...
template <typename RingT>
void evaluation_domain<RingT>::add_poly_Z(const RingT &coeff,
                                          vector<RingT> &H) const {
...
// TODO: not sure if we are technically dividing on coset, we might be using a
// misleading method name at the moment.
template <typename RingT>
...
template <typename RingT>
shared_ptr<evaluation_domain<RingT>> get_evaluation_domain(
    const size_t min_size) {
  shared_ptr<evaluation_domain<RingT>> shared_domain(
      new evaluation_domain<RingT>(min_size));
