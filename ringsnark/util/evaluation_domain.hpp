 ...
 *****************************************************************************
 ...
 in {0,...,|S-1|}, the idx-th Lagrange polynomial (wrt to subset S) is defined
 to be \f[   L_{idx,S}(z) := prod_{k \neq idx} (z - a_k) / prod_{k \neq idx}
 (a_{idx} - a_k)   \f] Note that, by construction: \f[   \forall j \neq idx:
 L_{idx,S}(a_{idx}) = 1  \text{ and }  L_{idx,S}(a_j) = 0   \f]
 *****************************************************************************
...
class evaluation_domain {
 ...
 public:
  ...
  /**
   * Add the coefficients of the vanishing polynomial of S to the coefficients
   * of the polynomial H.
   */
  void add_poly_Z(const RingT &coeff, std::vector<RingT> &H) const;

  /**
   * Multiply by the evaluation, on a coset of S, of the inverse of the
   * vanishing polynomial of S.
   */
  void divide_by_Z_on_coset(std::vector<RingT> &P) const;
};

template <typename RingT>
std::shared_ptr<evaluation_domain<RingT> > get_evaluation_domain(
    size_t min_size);
