...
linear_term<RingT> variable<RingT>::operator*(
    const integer_coeff_t int_coeff) const {
...
template <typename RingT>
linear_combination<RingT> variable<RingT>::operator+(
    const linear_combination<RingT> &other) const {
  linear_combination<RingT> result;

  result.add_term(*this);
  result.terms.insert(result.terms.begin(), other.terms.begin(),
                      other.terms.end());
...
template <typename RingT>
linear_term<RingT> operator*(const integer_coeff_t int_coeff,
                             const variable<RingT> &var) {
  return linear_term<RingT>(var, int_coeff);
}

template <typename RingT>
linear_term<RingT> operator*(const RingT &field_coeff,
                             const variable<RingT> &var) {
  return linear_term<RingT>(var, field_coeff);
}

template <typename RingT>
linear_combination<RingT> operator+(const integer_coeff_t int_coeff,
                                    const variable<RingT> &var) {
  return linear_combination<RingT>(int_coeff) + var;
}

template <typename RingT>
linear_combination<RingT> operator+(const RingT &field_coeff,
                                    const variable<RingT> &var) {
...
template <typename RingT>
linear_term<RingT> linear_term<RingT>::operator*(
    const integer_coeff_t int_coeff) const {
...
template <typename RingT>
linear_combination<RingT> linear_term<RingT>::operator+(
    const linear_combination<RingT> &other) const {
...
template <typename RingT>
linear_combination<RingT> linear_combination<RingT>::operator*(
    const integer_coeff_t int_coeff) const {
  return (*this) * RingT(int_coeff);
}

template <typename RingT>
RingT linear_combination<RingT>::evaluate(
    const std::vector<RingT> &assignment) const {
  RingT acc = RingT::zero();
  for (auto &lt : terms) {
    acc +=
        (lt.index == 0 ? RingT::one() : assignment.at(lt.index - 1)) * lt.coeff;
  }
  return acc;
}
...
linear_combination<RingT> linear_combination<RingT>::operator+(
  ...
  /* invariant: it1 and it2 always point to unprocessed items in the
   * corresponding linear combinations */
  while (it1 != this->terms.end() && it2 != other.terms.end()) {
    if (it1->index < it2->index) {
      ...
      /* it1->index == it2->index */
      result.terms.emplace_back(linear_term<RingT>(variable<RingT>(it1->index),
                                                   it1->coeff + it2->coeff));
      ++it1;
      ++it2;
    }
...
template <typename RingT>
bool linear_combination<RingT>::operator==(
    const linear_combination<RingT> &other) const {
...
template <typename RingT>
void linear_combination<RingT>::print(
...
template <typename RingT>
void linear_combination<RingT>::print_with_assignment(
    const std::vector<RingT> &full_assignment,
    const std::map<size_t, std::string> &variable_annotations) const {
  for (auto &lt : terms) {
    if (lt.index == 0) {
      printf("    1 * ");
...
template <typename RingT>
linear_combination<RingT>::linear_combination(
    const std::vector<linear_term<RingT>> &all_terms) {
  if (all_terms.empty()) {
    return;
  }
  ...
  std::sort(terms.begin(), terms.end(),
            [](linear_term<RingT> a, linear_term<RingT> b) {
