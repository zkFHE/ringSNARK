...
template <typename RingT>
r1cs_constraint<RingT>::r1cs_constraint(
    const std::initializer_list<linear_combination<RingT> > &A,
...
template <typename RingT>
bool r1cs_constraint<RingT>::operator==(
    const r1cs_constraint<RingT> &other) const {
...
void dump_r1cs_constraint(
    const r1cs_constraint<RingT> &constraint,
    const r1cs_variable_assignment<RingT> &full_variable_assignment,
    const std::map<size_t, std::string> &variable_annotations) {
  printf("terms for a:\n");
  constraint.a.print_with_assignment(full_variable_assignment,
                                     variable_annotations);
  printf("terms for b:\n");
  constraint.b.print_with_assignment(full_variable_assignment,
                                     variable_annotations);
  printf("terms for c:\n");
  constraint.c.print_with_assignment(full_variable_assignment,
                                     variable_annotations);
}

template <typename RingT>
bool r1cs_constraint_system<RingT>::is_satisfied(
  ...
  for (size_t c = 0; c < constraints.size(); ++c) {
    ...
    if (!(ares * bres == cres)) {
      ...
      dump_r1cs_constraint(constraints[c], full_variable_assignment,
                           variable_annotations);
#endif  // DEBUG
      return false;
    }
...
template <typename RingT>
void r1cs_constraint_system<RingT>::add_constraint(
...
template <typename RingT>
void r1cs_constraint_system<RingT>::add_constraint(
    const r1cs_constraint<RingT> &c, const std::string &annotation) {
#ifdef DEBUG
...
void r1cs_constraint_system<RingT>::swap_AB_if_beneficial() {
  // libff::enter_block("Call to
  // r1cs_constraint_system::swap_AB_if_beneficial");
  ...
  */
  ...
  //        libff::leave_block("Call to
  //        r1cs_constraint_system::swap_AB_if_beneficial");
}

template <typename RingT>
bool r1cs_constraint_system<RingT>::operator==(
    const r1cs_constraint_system<RingT> &other) const {
...
template <typename RingT>
void r1cs_constraint_system<RingT>::report_linear_constraint_statistics()
    const {
#ifdef DEBUG
  for (size_t i = 0; i < constraints.size(); ++i) {
    ...
    if (a_is_const || b_is_const) {
      ...
      printf("%s\n",
             (it == constraint_annotations.end() ? FMT("", "constraint_%zu", i)
                                                 : it->second)
                 .c_str());
    }
  }
