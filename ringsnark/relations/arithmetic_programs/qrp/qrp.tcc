...
qrp_instance<RingT>::qrp_instance(
    ...
    const size_t num_variables, const size_t degree, const size_t num_inputs,
...
template <typename RingT>
bool qrp_instance<RingT>::is_satisfied(
    const qrp_witness<RingT> &witness) const {
  ...
  const qrp_instance_evaluation<RingT> eval_qrp_inst(
      this->domain, this->num_variables(), this->degree(), this->num_inputs(),
      t, std::move(At), std::move(Bt), std::move(Ct), std::move(Ht), Zt);
  return eval_qrp_inst.is_satisfied(witness);
}

template <typename RingT>
qrp_instance_evaluation<RingT>::qrp_instance_evaluation(
    const std::shared_ptr<evaluation_domain<RingT>> &domain,
    const size_t num_variables, const size_t degree, const size_t num_inputs,
    const RingT &t, const std::vector<RingT> &At, const std::vector<RingT> &Bt,
    const std::vector<RingT> &Ct, const std::vector<RingT> &Ht, const RingT &Zt)
    :
...
qrp_instance_evaluation<RingT>::qrp_instance_evaluation(
    const std::shared_ptr<evaluation_domain<RingT>> &domain,
    const size_t num_variables, const size_t degree, const size_t num_inputs,
    const RingT &t, std::vector<RingT> &&At, std::vector<RingT> &&Bt,
    std::vector<RingT> &&Ct, std::vector<RingT> &&Ht, const RingT &Zt)
    : num_variables_(num_variables),
      degree_(degree),
      num_inputs_(num_inputs),
      domain(domain),
...
template <typename RingT>
bool qrp_instance_evaluation<RingT>::is_satisfied(
    const qrp_witness<RingT> &witness) const {
  ...
  if (this->At.size() != this->num_variables() + 1 ||
      this->Bt.size() != this->num_variables() + 1 ||
  ...
  ans_A =
      ans_A +
      inner_product<RingT>(
          this->At.begin() + 1, this->At.begin() + 1 + this->num_variables(),
          witness.coefficients_for_ABCs.begin(),
          witness.coefficients_for_ABCs.begin() + this->num_variables());
  ans_B =
      ans_B +
      inner_product<RingT>(
          this->Bt.begin() + 1, this->Bt.begin() + 1 + this->num_variables(),
          witness.coefficients_for_ABCs.begin(),
          witness.coefficients_for_ABCs.begin() + this->num_variables());
  ans_C =
      ans_C +
      inner_product<RingT>(
          this->Ct.begin() + 1, this->Ct.begin() + 1 + this->num_variables(),
          witness.coefficients_for_ABCs.begin(),
          witness.coefficients_for_ABCs.begin() + this->num_variables());
  ans_H = ans_H + inner_product<RingT>(
                      this->Ht.begin(), this->Ht.begin() + this->degree() + 1,
...
template <typename RingT>
qrp_witness<RingT>::qrp_witness(
    const size_t num_variables, const size_t degree, const size_t num_inputs,
    const RingT &d1, const RingT &d2, const RingT &d3,
