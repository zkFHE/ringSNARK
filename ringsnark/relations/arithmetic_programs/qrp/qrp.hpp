 ...
 *****************************************************************************
 ...
 QRPs are defined in \[GNSV21], and are very similar to QAPs (Quadratic
Arithmetic Programs) \[GGPR13].
...
class qrp_instance {
 ...
 public:
  ...
  qrp_instance(
      ...
      size_t num_variables, size_t degree, size_t num_inputs,
...
/**
 * A QRP instance evaluation is a QRP instance that is evaluated at a ring
 * element t.
 *
 * Specifically, the datastructure stores:
 * - a choice of domain (corresponding to a certain subset of the ring);
...
class qrp_instance_evaluation {
 ...
 public:
  ...
  qrp_instance_evaluation(
      const std::shared_ptr<evaluation_domain<RingT> > &domain,
      size_t num_variables, size_t degree, size_t num_inputs, const RingT &t,
      const std::vector<RingT> &At, const std::vector<RingT> &Bt,
      const std::vector<RingT> &Ct, const std::vector<RingT> &Ht,
      const RingT &Zt);

  qrp_instance_evaluation(
      const std::shared_ptr<evaluation_domain<RingT> > &domain,
      size_t num_variables, size_t degree, size_t num_inputs, const RingT &t,
      std::vector<RingT> &&At, std::vector<RingT> &&Bt, std::vector<RingT> &&Ct,
      std::vector<RingT> &&Ht, const RingT &Zt);

  qrp_instance_evaluation(const qrp_instance_evaluation<RingT> &other) =
      default;

  qrp_instance_evaluation(qrp_instance_evaluation<RingT> &&other) noexcept =
      default;

  qrp_instance_evaluation &operator=(
      const qrp_instance_evaluation<RingT> &other) = default;

  qrp_instance_evaluation &operator=(
      qrp_instance_evaluation<RingT> &&other) noexcept = default;
...
class qrp_witness {
 ...
 public:
  ...
  qrp_witness(size_t num_variables, size_t degree, size_t num_inputs,
