...
class variable {
 public:
  ...
  linear_combination<RingT> operator+(
...
template <typename RingT>
linear_term<RingT> operator*(const RingT &ring_coeff,
                             const variable<RingT> &var);
...
template <typename RingT>
linear_combination<RingT> operator+(const RingT &ring_coeff,
                                    const variable<RingT> &var);
...
/**
 * A linear combination represents a formal expression of the form "sum_i
 * coeff_i * x_{index_i}".
 */
template <typename RingT>
class linear_combination {
 public:
  ...
  friend std::ostream &operator<< <RingT>(std::ostream &out,
                                          const linear_combination<RingT> &lc);

  friend std::istream &operator>>
      <RingT>(std::istream &in, linear_combination<RingT> &lc);
};
